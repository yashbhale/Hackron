...
// POST handler for /api/predict (or similar) remains unchanged
export async function POST(req) {
  try {
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to fetch cluster data" }),
        { status: 500 }
      );
    }
    ...
    if (!Array.isArray(cluster_centers)) {
      return new Response(
        JSON.stringify({ error: "Invalid cluster data received" }),
        { status: 500 }
      );
    }

    // Process cluster data with geolocation caching
    const clusterInsights = await Promise.all(
      cluster_centers.map(async (coords, index) => {
