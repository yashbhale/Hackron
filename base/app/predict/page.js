    ...
    <ul className="mt-2 space-y-3">
      {result.insights?.map((insight, index) => (
        ...
        {result?.insights?.length > 0 && leaflet && (
          <div className="flex-1 h-[500px] lg:h-auto rounded-lg overflow-hidden shadow-md mt-6">
            <MapViewer />
          </div>
        )}
      </div>
...
const OptimizedMap = ({ insights, leaflet }) => {
  ...
  return (
    <MapContainer
      center={[
        insights[0]?.latitude || 19.076,
        insights[0]?.longitude || 72.8777,
      ]}
      zoom={12}
        ...
        >
          <Popup className="text-sm font-semibold">
            <div>
              ...
              <p>
                <strong>Area:</strong> {insight.area}
              </p>
              <p>
                <strong>Latitude:</strong> {insight.latitude}
              </p>
              <p>
                <strong>Longitude:</strong> {insight.longitude}
              </p>
              <p>
                <strong>Rental Cost:</strong> {insight.rentalcost}
              </p>
              <p>
                <strong>Travel Cost:</strong> {insight.travelingcost}
              </p>
              <p>
                <strong>Size:</strong> {insight.size}
              </p>
            </div>
          </Popup>
        </Marker>
